...
public static class NetTests
{
    ...
    /// </summary>
    public static volatile LogSink Log = LogManager.GetSource("Tests", "Network");
    ...
    internal static void Client()
    {
        ...
        ConsoleCommands.Manager.RegisterCommand(typeof(NetTests), null, out _, out _);
    ...
    internal static void Server()
    {
        ...
        server.Connected += peer =>
        {
            ...
            peer.RegisterHandler<HelloMessage>(msg => Log.Info($"Received message: &1{msg}&r"));
            
            peer.RegisterHandler<LogTimeMessage>(msg =>
            {
                var timeTicks = peer.Time.Ticks + (DateTime.UtcNow.Ticks - msg.SentTicks);
                var timeMs = (float)(timeTicks / TimeSpan.TicksPerMillisecond);
            ...
            if (manager is null)
            {
                Log.Error($"Entity manager not found for peer &1{peer.EndPoint}&r");
                return;
            }
    ...
    private static void Ping(ref CommandContext<object> ctx)
    {
        ...
        else if (server != null)
        {
            ...
            ctx.SetOkText($"Ping: {peer.Ping.Last} ms ({peer.Ping.Average} ms avg)");
        }
        else
        {
    ...
    private static void Time(ref CommandContext<object> ctx)
    {
        ...
        else if (server != null)
        {
            ...
            var timeTicks = peer.Time.Ticks;
            var timeMs = peer.Time.Time * 1000f;
