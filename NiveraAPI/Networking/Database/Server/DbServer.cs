...
public class DbServer : Service
{
    ...
    public override void Start()
    {
        ...
        if (Collection is INetworkServer server)
            server.ProvidedServices =
                (server.ProvidedServices ?? Array.Empty<Type>()).Concat([typeof(DbUser)]).ToArray();
    ...
    public override void Stop()
    {
        ...
        if (Collection is INetworkServer server && server.ProvidedServices != null)
            server.ProvidedServices = server.ProvidedServices.Except([typeof(DbUser)]).ToArray();
        else if (Collection is RestServer restServer)
